$(document).ready(function() {
                  ...
                  success: function(result){
                    ...
                    if (result.resultadoInmuebles.length > 0) {
                      ...
                      for (var i = inmuebles.length - 1; i >= 0; i--) {
                        ...
                        mensaje_alerta +=         '<a class="btn btn-primary" role="button" data-toggle="collapse" href="#collapseExample" aria-expanded="false" aria-controls="collapseExample">+</a>';
                        mensaje_alerta +=         '<div class="collapse" id="collapseExample">';
                        mensaje_alerta +=           '<div class="well">';
                        mensaje_alerta +=              '<form id="form'+inmuebles[i][0].id+'" action="contactar-propietario/" method="post">';
                        mensaje_alerta +=                '<input name="_token" type="hidden" value = "' + inmuebles[i][1]+'" id="token">';
                        mensaje_alerta +=                '<input type="hidden" name="_method" id="method" value="POST">';
                        mensaje_alerta +=                '<input name="usuario_id" type="hidden" value = "'+inmuebles[i][0].usuario_id+'">';
                        mensaje_alerta +=                '<input name="inmueble_id" type="hidden" value = "'+inmuebles[i][0].id+'">';
                        mensaje_alerta +=                '<button type="submit" class="btn btn-success btn-sm btn-contact" data-inmuebleid = "'+inmuebles[i][0].id+'">';
                        mensaje_alerta +=                  '<i class="fa fa-refresh fa-spin" id="i-spinner-'+inmuebles[i][0].id+'" style="display: none"></i>';
                        mensaje_alerta +=                  '<i class="fa fa-handshake-o" id="i-shown'+inmuebles[i][0].id+'" aria-hidden="true"></i><strong>Contactar Propietario</strong>';
                        mensaje_alerta +=                  '<span id="txt-contacto-'+inmuebles[i][0].id+'"><strong>Contactar Propietario</strong></span>';
    ...
    $('.btn-contact').bind('click', function (e) {
      ...
      $('#txt-contacto-'+inmuebles[i][0].id).html('Procesando...');
      var timeOutId = 0;
      var promise = promiseAjax(url, 'json', 'POST', token, data);
      promise.done(function (result) {
        $('#i-spinner' + inmueble_id).hide();
        $('#i-shown' + inmueble_id).show();
        $('#txt-contacto-' + inmuebles[i][0].id).html('Contactar Propietario');
        /* Sending the success message */
        showMessage(result.message, 'Enhorabuena', 'info');
      }).catch(function (fail) {
        var errors = errorResponse(fail);
